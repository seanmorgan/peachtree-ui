  ...
  return (
    ...
    >
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              min={50}
              max={110}
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-900 shadow-sm placeholder:text-slate-300 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder:text-slate-600 dark:focus:border-indigo-500"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5">
        ...
        {!forecast ? (
          <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-700 py-8 text-center text-slate-400 dark:text-slate-600">
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="col-span-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 p-3 text-center">
                ...
                {rankInfo.map(r => (
                  <div key={r.key} className="flex items-center gap-3 rounded-lg px-3 py-2 bg-amber-50/60 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/10">
                    ...
                    <span
                      ...
                      style={{ background: r.rank === 1 ? '#f59e0b' : r.rank <= 3 ? '#6366f1' : '#94a3b8' }}
                    >
                      {getOrdinal(r.rank)}
                      {r.rank === 1 ? ' 🔥' : ''}
                    </span>
            ...
            <button
              ...
              className="w-full rounded-lg border border-slate-200 py-2 text-sm text-slate-400 hover:text-slate-600 hover:border-slate-300 dark:border-slate-700 dark:text-slate-500 dark:hover:border-slate-600 dark:hover:text-slate-400 transition-colors"
            >
              Clear forecast
            </button>
          </>
