        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. 75"
              min={50} max={110}
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-900 shadow-sm placeholder:text-slate-300 focus:border-peachtree-400 focus:outline-none focus:ring-2 focus:ring-peachtree-500/20"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="col-span-2 rounded-xl bg-slate-50 p-3 text-center">
                <p className="text-xs text-slate-400 mb-1">Stress Score</p>
            ...
            <div>
              ...
              <div className="space-y-2">
                {rankInfo.map(r => (
                  <div key={r.key} className="flex items-center gap-3 rounded-lg px-3 py-2 bg-amber-50/60 border border-amber-100">
                    <span className="h-2.5 w-2.5 rounded-full flex-shrink-0" style={{ background: r.color }} />
            ...
            <button
              onClick={() => { setTempInput(''); setDewInput(''); setWindInput(''); onForecastChange(null) }}
              className="w-full rounded-lg border border-slate-200 py-2 text-sm text-slate-400 hover:text-slate-600 hover:border-slate-300 transition-colors"
            >
              Clear forecast
            </button>
          </>
